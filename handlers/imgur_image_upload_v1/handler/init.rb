...
class ImgurImageUploadV1
  ...
  def execute() 
    access_token = get_access_token(@info_values['client_id'], @info_values['client_secret'], @info_values['refresh_token'])
    ...
    params = {
      'image' => attachment_value.content,
      'type' => 'file'
    }

    begin
      result = RestClient.post "https://api.imgur.com/3/image", params, :authorization => "Bearer #{access_token}"
  ...
  def get_access_token(client_id, client_secret, refresh_token)
    ...
    result = RestClient.post("https://api.imgur.com/oauth2/token",params)
    json = JSON.parse(result)
